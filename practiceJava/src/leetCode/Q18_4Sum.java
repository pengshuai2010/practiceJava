...
public class Q18_4Sum {
    public static void main(String[] args) {
        Q18_4Sum solution = new Q18_4Sum();
        int[][] inputs = new int[][]{{1, 0, -1, 0, -2, 2},
                {91277418, 66271374, 38763793, 4092006, 11415077, 60468277, 1122637, 72398035, -62267800, 22082642, 60359529,
                        ...
                        -47800372, 39958829, -95981751, -71017359, -18397211, 27941418, -34699076, 74174334, 96928957, 44328607, 49293516,
                        -39034828, 5945763, -47046163, 10986423, 63478877, 30677010, -21202664, -86235407, 3164123, 8956697, -9003909, -18929014, -73824245}};
        int[] targets = new int[]{0, -236727523};
        for (int i = 0; i < inputs.length; i++) {
            System.out.println(solution.fourSum(inputs[i], targets[i]));
        }
    }
    ...
    public List<List<Integer>> fourSum(int[] nums, int target) {
        ...
        for (int i = 0; i < nums.length - 3; i++) {
            ...
            for (int j = i + 1; j < nums.length - 2; j++) {
                ...
                if (innerTarget > nums[p2] * 2 || innerTarget < nums[p1] * 2)// trimming branches
                    continue;
                while (p1 < p2) {
                    if (nums[p1] + nums[p2] < innerTarget) {
                        do {
                            p1++;
                        } while (p1 < p2 && nums[p1] == nums[p1 - 1]);
                    } else if (nums[p1] + nums[p2] > innerTarget) {
                        do {
                            p2--;
                        } while (p1 < p2 && nums[p2] == nums[p2 + 1]);
                    } else {
                        solutions.add(Arrays.asList(nums[i], nums[j], nums[p1], nums[p2]));
                        do {
                            p1++;
                        } while (p1 < p2 && nums[p1] == nums[p1 - 1]);
                        do {
